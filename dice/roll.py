...
def dice( rdl : list):
    ...
    while (di < len(rdl)):
        if '!' in rdl[di]:
            dicer = [rdl[di]]
            try:
                ed2r = rdl[di].split('!', 1)[0] #Remove exploding dice flag
                rollt, maxn = map(int, ed2r.split('d')) # Decide what and how many times to roll
            except ValueError:
                dicer.append('Error with dice.')
            try:
                valueerror = []
                if rollt > 255:
                ...
                if maxn < 2:
                    valueerror.append('Too low of dice')
                    raise ValueError('To low of dice')
            except ValueError:
